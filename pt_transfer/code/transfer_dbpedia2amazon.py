...
from peft import LoraConfig, get_peft_model, TaskType, PeftModel, PromptTuningConfig
...
def parse_args():
    ...
    parser.add_argument('--source_model', type=str, default='pt-dbpedia_model__epoch10_lr0.0001_data25000', help='Pre-trained model name')
    parser.add_argument('--dataset_name', type=str, default='amazon_polarity', help='Dataset name')
    ...
    parser.add_argument('--num_eval', type=int, default=25000, help='Number of test samples')
...
def load_and_tokenize_data(model_name, dataset_name, seed, train_sample_size, eval_sample_size):
    ...
    def tokenize_function(examples):
        if dataset_name == "sst2":
            ...
            text_key = "text"
        return tokenizer(examples[text_key], max_length=492, padding="max_length", truncation=True)
...
def create_model(base_model, source_model, num_label = 2):
    base = AutoModelForSequenceClassification.from_pretrained(base_model, num_labels=14)
    ...
    model_2 = get_peft_model(model, prompt_tuning_config)
    model_2.prompt_encoder = model_1.prompt_encoder
    return model_2
...
def main():
    ...
    # Log file setup
    log_file = 'transfer_dbpedia2amazon_log_epoch10_lr1e-4.txt'
    ...
    for epoch, lr in itertools.product(epochs, learning_rates):
        ...
        model.save_pretrained(f"pt-transfer_dbpedia2amazon__epoch{epoch}_lr{lr}_data{args.num_train}")
